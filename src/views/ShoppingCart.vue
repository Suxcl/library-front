              ...
              <div class="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
                  <div class="flex flex-col justify-start items-start dark:bg-gray-900 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
                      <p class="text-lg md:text-xl dark:text-white font-semibold leading-6 xl:leading-5 text-gray-800">Customer’s Cart</p>
                          <div v-for="meal in products" :key="meal.idMeal" class="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
                          <div class="pb-4 md:pb-8 w-full md:w-40">
                              <img class="w-full hidden md:block" :src="meal.strMealThumb" :alt="meal.strMeal"  />
                          </div>
                          <div class="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
                              <div class="w-full flex flex-col justify-start items-start space-y-8">
                                  <h3 class="text-xl dark:text-white xl:text-2xl font-semibold leading-6 text-gray-800">{{ meal.strMeal }}</h3>
                                  <div class="flex justify-start items-start flex-col space-y-2">
                                      <p class="text-base dark:text-white xl:text-lg leading-6">{{ meal.idMeal }}$</p>
                                      <p class="text-sm dark:text-white leading-none text-gray-800"><span class="dark:text-gray-400 text-gray-700">Style: </span> {{ meal.strCategory }}</p>
                                  </div>
                              </div>
                              <div class="flex justify-between space-x-8 items-start w-full">
                                  <p class="text-base dark:text-white xl:text-lg leading-6"></p>
                                  <button  @click=" deleteFromCart(meal) " class=" text-black dark:text-white font-bold py-2 px-4 rounded">
  ...
  deleteFromCart(item)
{
  ...
  var index = cart.findIndex(x => x.strMeal === item.strMeal);
      cart.splice(index, 1);
      localStorage.setItem('products', JSON.stringify(cart));
      this.products = JSON.parse(localStorage.getItem('products'));
      console.log(localStorage.products)
      toast.info(item.strMeal + ' deleted from cart');
},
  day()
  {
      var today = new Date();
